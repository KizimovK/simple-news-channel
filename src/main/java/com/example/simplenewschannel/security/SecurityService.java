...
public class SecurityService {
    ...
    public RefreshTokenResponse refreshToken(RefreshTokenRequest refreshTokenRequest) {
        ...
        return refreshTokenService.findByRefreshToken(refreshToken).map(refreshTokenService::checkRefreshToken).map(RefreshToken::getUserId).map(userId -> {
            refreshTokenService.deleteByUserId(userId);
            User tokenOwner = userRepository.findById(userId).orElseThrow(() -> new RefreshTokenException("Exception trying to get token for user id: " + userId));
